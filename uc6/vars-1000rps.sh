...
# Message file
$(echo ${JAVA_OPTS} | grep -q 'DmessagesFile=')
if [ $? -ne 0 ]; then JAVA_OPTS="${JAVA_OPTS} -DmessagesFile=messages.csv"; fi
...
if [ $? -ne 0 ]; then JAVA_OPTS="${JAVA_OPTS} -DmaxHostConcurrentUsers=1000"; fi
# for open model
$(echo ${JAVA_OPTS} | grep -q 'DmaxHostIncrementUsersPerSec=')
if [ $? -ne 0 ]; then JAVA_OPTS="${JAVA_OPTS} -DmaxHostIncrementUsersPerSec=1000"; fi
